...
const demoCredentials: Array<SessionRequest & { roleLabel: string; accessSummary: string }> = [
  {
    ...
    pin: "1357",
    roleLabel: "Analyst",
    accessSummary: "Fraud confirm and legit decisions",
  },
  {
    analyst_name: "lead.analyst",
    pin: "2468",
    roleLabel: "Lead Analyst",
    accessSummary: "Escalation and queue ownership",
  },
  {
    analyst_name: "fraud.manager",
    pin: "9999",
    roleLabel: "Manager",
    accessSummary: "Full review and escalation access",
  },
];
...
function statusTone(status: AlertRecord["status"]): string {
...
export default function App() {
  ...
  async function refreshData(activeSession: AnalystSession = session as AnalystSession) {
    ...
    try {
      ...
      const [alertsResponse, metricsResponse] = await Promise.all([
        getAlerts(activeSession.token),
        getMetrics(activeSession.token),
      ]);
      setAlerts(alertsResponse.alerts);
      setMetrics(metricsResponse);
      setSelectedAlertId((current) => {
        if (current && alertsResponse.alerts.some((alert) => alert.id === current)) {
          return current;
        }
      ...
      if (isUnauthorized(message)) {
        logout("Session expired. Sign back in to continue.");
      }
    }
  }
  ...
  async function handleScore(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatusMessage(
        `Scored ${transaction.external_id}: ${response.risk_band.toUpperCase()} risk at ${Math.round(response.risk_score * 100)}%.`,
      );
      setTransaction(defaultTransaction());
      await refreshData(session);
      if (response.alert_id) {
        setSelectedAlertId(response.alert_id);
      }
    } catch (requestError) {
      ...
      <div className="app-shell login-shell">
        <section className="login-hero">
          <div className="hero-copy-block">
            ...
            <p className="hero-copy">
              I rebuilt the entry flow so it feels closer to a guarded internal console than a plain demo page.
            </p>
          </div>
          <div className="signal-board">
            <article className="signal-card signal-card-accent">
              <span>Live Signal</span>
              <strong>Real-time scoring, alert creation, and audit updates</strong>
            </article>
            <article className="signal-card">
              <span>Role Split</span>
              <strong>Analyst, lead analyst, and manager sessions with different actions</strong>
            </article>
            <article className="signal-card">
              <span>Ops Focus</span>
              <strong>Latency, review precision, amount shift, and international rate tracking</strong>
            </article>
          </div>
          <div className="credential-rack">
            {demoCredentials.map((credential) => (
              <button
                type="button"
                key={credential.analyst_name}
                className="credential-card"
                onClick={() => setLoginForm({ analyst_name: credential.analyst_name, pin: credential.pin })}
              >
        ...
        <section className="login-panel panel">
          <div className="panel-header">
            <h2>Sign In</h2>
            <span>Seeded demo analysts</span>
          </div>
          <form className="login-form" onSubmit={handleLogin}>
            <label>
              Analyst Name
              <input
                value={loginForm.analyst_name}
                onChange={(event) => setLoginForm({ ...loginForm, analyst_name: event.target.value })}
              />
            </label>
            <label>
              PIN
            ...
            <button type="submit" disabled={isBusy}>
              {isBusy ? "Signing In..." : "Unlock Console"}
            </button>
          </form>
  ...
  return (
    <div className="app-shell">
      <header className="hero hero-dashboard">
        <div className="hero-copy-block">
          <p className="eyebrow">Bank-Style Fraud Operations</p>
          <h1>FraudShield Analyst Console</h1>
          <p className="hero-copy">
            I shaped this dashboard around the full workflow: score a transaction, inspect the alert, explain why it
            was flagged, and leave a review trail that somebody else can audit later.
          </p>
        ...
        <div className="hero-status">
          <div className="session-meta">
            ...
            <span className="status-pill neutral">{formatRole(session.role)}</span>
          </div>
          <p>{statusMessage}</p>
          <div className="analyst-chip-row">
            <span className="analyst-chip">Signed in as {session.analyst_name}</span>
      ...
      <section className="ops-strip">
        <article className="ops-card ops-card-strong">
          <span>Decision Rights</span>
          <strong>{canEscalate ? "Escalation enabled for this session." : "Escalation restricted to lead roles."}</strong>
        </article>
        <article className="ops-card">
          <span>Review Velocity</span>
          <strong>{metrics?.reviewed_alerts ?? reviewedAlerts} cases reviewed in this run state</strong>
        </article>
        <article className="ops-card">
          <span>Risk Posture</span>
          <strong>{Math.round((metrics?.average_risk_score ?? 0) * 100)}% average risk score across stored transactions</strong>
        </article>
      </section>
      ...
      <section className="metrics-grid">
        ...
        <MetricCard
          label="Intl. Rate"
          value={`${Math.round((metrics?.recent_international_rate ?? 0) * 100)}% / ${Math.round(
            (metrics?.baseline_international_rate ?? 0) * 100,
          )}% base`}
          tone="emerald"
        />
      </section>
      ...
      <main className="workspace">
        <section className="panel panel-form">
          <div className="panel-header">
            ...
            <span>Authenticated real-time decisioning</span>
          </div>
          ...
          <form className="score-form" onSubmit={handleScore}>
            <label>
              ...
              <input
                ...
                onChange={(event) => setTransaction({ ...transaction, external_id: event.target.value })}
              />
            </label>
            <label>
              Amount
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => setTransaction({ ...transaction, merchant_name: event.target.value })}
              />
            </label>
            <label>
              Category
              <select
                ...
                onChange={(event) => setTransaction({ ...transaction, merchant_category: event.target.value })}
              >
            ...
            <label>
              Entry Mode
              <select
                value={transaction.entry_mode}
                onChange={(event) => setTransaction({ ...transaction, entry_mode: event.target.value })}
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => setTransaction({ ...transaction, country: event.target.value.toUpperCase() })}
              />
            </label>
            <label>
              IP Risk Score
              <input
                ...
                max="1"
                step="0.01"
                value={transaction.ip_risk_score}
                onChange={(event) => setTransaction({ ...transaction, ip_risk_score: Number(event.target.value) })}
              />
            </label>
            <label>
              Velocity 1h
              <input
                type="number"
                value={transaction.velocity_1h}
                onChange={(event) => setTransaction({ ...transaction, velocity_1h: Number(event.target.value) })}
              />
            </label>
            <label>
              Velocity 24h
              <input
                type="number"
                value={transaction.velocity_24h}
                onChange={(event) => setTransaction({ ...transaction, velocity_24h: Number(event.target.value) })}
              />
            </label>
            <label>
              Account Age
              <input
                ...
                onChange={(event) => setTransaction({ ...transaction, account_age_days: Number(event.target.value) })}
              />
            </label>
            <label>
              Email Age
              <input
                type="number"
                value={transaction.email_age_days}
                onChange={(event) => setTransaction({ ...transaction, email_age_days: Number(event.target.value) })}
              />
            </label>
            <label>
              Customer Tenure
              <input
                type="number"
                value={transaction.customer_tenure_days}
                onChange={(event) =>
                  setTransaction({ ...transaction, customer_tenure_days: Number(event.target.value) })
                }
              />
            </label>
            <label className="checkbox-row">
              <input
                type="checkbox"
                checked={transaction.card_present}
                onChange={(event) => setTransaction({ ...transaction, card_present: event.target.checked })}
              />
              Card present
            </label>
            <label className="checkbox-row">
              <input
                ...
                onChange={(event) => setTransaction({ ...transaction, is_international: event.target.checked })}
              />
              International
            </label>
            <button type="submit" disabled={isBusy}>
              {isBusy ? "Scoring..." : "Score Transaction"}
            </button>
          </form>
        </section>
        ...
        <section className="panel panel-alerts">
          ...
          <div className="alert-list">
            {alerts.map((alert) => (
              ...
              >
                <div className="alert-card-header">
                  <div>
                    <span className="alert-merchant">{alert.transaction.merchant_name}</span>
                    <small>{formatCurrency(alert.transaction.currency, alert.transaction.amount)}</small>
                  </div>
                  <span className={`risk-pill ${riskTone(alert.transaction.risk_band)}`}>
                    {alert.transaction.risk_band}
                  </span>
                </div>
            ...
            {alerts.length === 0 ? (
              <p className="empty-state">No alerts yet. Score a risky transaction to create one.</p>
            ) : null}
          </div>
        </section>
        ...
        <section className="panel panel-detail">
          <div className="panel-header">
            <h2>Alert Detail</h2>
            <span>Explanation and audit trail</span>
          </div>
          {selectedAlert ? (
            <div className="detail-stack">
              <div className="detail-card hero-detail">
                <div className="detail-topline">
                  <div>
                    <h3>{selectedAlert.transaction.merchant_name}</h3>
                    <small>{selectedAlert.reason_summary}</small>
                  </div>
                  <span className={`risk-pill ${riskTone(selectedAlert.transaction.risk_band)}`}>
                    {Math.round(selectedAlert.transaction.risk_score * 100)}% {selectedAlert.transaction.risk_band}
                  </span>
                </div>
                <div className="detail-snapshot">
                  <div>
                    <span>Amount</span>
                    <strong>{formatCurrency(selectedAlert.transaction.currency, selectedAlert.transaction.amount)}</strong>
                  </div>
                  <div>
                    <span>Country</span>
                    <strong>{selectedAlert.transaction.country}</strong>
                  </div>
                  <div>
                    <span>Latency</span>
                    <strong>{selectedAlert.transaction.latency_ms} ms</strong>
                  </div>
                  <div>
                    <span>Created</span>
                    <strong>{formatTimestamp(selectedAlert.created_at)}</strong>
                  </div>
                </div>
                <div className="factor-list factor-list-bars">
                  {selectedAlert.transaction.top_factors.map((factor) => {
              ...
              <div className="detail-grid">
                <div className="detail-card">
                  <div className="detail-topline">
                    ...
                    <span className="analyst-chip">{session.analyst_name}</span>
                  </div>
                  <label>
                    Notes
                  ...
                  <div className="action-row">
                    <button type="button" className="confirm" disabled={isBusy} onClick={() => void handleDecision("confirm_fraud")}>
                      Confirm Fraud
                    </button>
                    <button type="button" className="legit" disabled={isBusy} onClick={() => void handleDecision("mark_legit")}>
                      Mark Legit
                    </button>
                    <button
                      type="button"
                      className="escalate"
                      disabled={!canEscalate || isBusy}
                      onClick={() => void handleDecision("escalate")}
                      title={canEscalate ? "Escalate alert" : "Escalation requires a lead analyst or manager session"}
                    >
                      Escalate
                    </button>
                  </div>
                      ...
                      <div key={decision.id} className="decision-row">
                        ...
                        <div className="decision-body">
                          <div className="decision-heading">
                            <span className={`status-tag ${statusTone(decision.decision === "escalate" ? "escalated" : "resolved")}`}>
                              {decision.decision}
                            </span>
